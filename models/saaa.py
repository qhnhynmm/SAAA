...
class CoAttention(nn.Module):
    def __init__(self, config):
        ...
        self.v_conv = nn.Linear(config.D_VISION, config.D_MODEL, bias=False)
    ...
    def forward(self, v, q):
        ...
        v = v.unsqueeze(1)
        q = q.unsqueeze(1)
        diff = v.size(2) - q.size(2)
        q = torch.nn.functional.pad(q, (0, 0, 0, diff))
        x = self.relu(v + q)
